...
void f3d_lcd_fillSection(int u, int v, uint16_t color) {
  uint8_t y;
  uint16_t x[ST7735_width];
  for (y = 0; y < v; y++) x[y] = color;
  f3d_lcd_setAddrWindow (0,ST7735_height-u,ST7735_width-1,ST7735_height-u + 4,MADCTLGRAPHICS);
  for (y=0;y<ST7735_height; y++) {
    f3d_lcd_pushColor(x,ST7735_width);
  }
}
